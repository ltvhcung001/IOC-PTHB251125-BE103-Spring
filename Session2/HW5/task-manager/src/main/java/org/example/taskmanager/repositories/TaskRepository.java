...
public class TaskRepository {
    ...
    public Task save(String id, Task task) {
        Task taskGetByID = findById(id);
        if (taskGetByID == null) {
            return null;
        }
        taskGetByID.setTitle(task.getTitle());
        taskGetByID.setDescription(task.getDescription());
        taskGetByID.setPriority(task.getPriority());
        taskGetByID.setAssignedTo(task.getAssignedTo());
        return taskGetByID;
    }
    ...
    public boolean deleteById(String id) {
        ...
        return tasks.remove(taskGetByID);
    }
}
